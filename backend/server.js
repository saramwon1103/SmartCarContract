...
// NOTE: `app.listen` moved to end of file after all route handlers so
// the server starts after routes are registered.
...
app.post("/api/contracts/onchain/create", async (req, res) => {
  try {
    ...
    const sql = `INSERT INTO Contracts (ContractId, CarId, UserId, OwnerId, Type, StartDate, EndDate, Deposit, TotalPrice, Status, TXHash, ContractAddress)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
